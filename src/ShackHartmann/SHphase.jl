...
"Build SH diversity using the quadratic functions"
struct Quadratic <: Alg end
"Build SH diversity using the linear functions"
struct Linear <: Alg end
...
function SHdiversity(size::Real, cellsize, celloffset, α, method)
    return SHdiversity((size, size), cellsize, celloffset, α, method)
end
...
function centresarray(cellind, e₁, e₂, o)
    ...
    cij = [
        (i + 0.5) * e₁ + (j + 0.5) * e₂ .+ o for i in UnitRange(ijrange[1]...),
        j in UnitRange(ijrange[2]...)
    ]
    return cij
end
...
"""
function orthIndexes(
    arrsize::Tuple{Int64,Int64}, cellsize, gridorigin::Tuple=(0.0, 0.0), α=0
)
    ind = zeros(arrsize..., 2)
    R = rotationmatrix(-α)
...
function orthIndexes(arrsize, cellsize, gridorigin::Real, args...)
    return orthIndexes(arrsize, cellsize, (gridorigin, gridorigin), args...)
end

function orthIndexes(arrsize::Int64, cellsize, args...)
    return orthIndexes((arrsize, arrsize), cellsize, args...)
end
...
function centre(i, j, cellsize, gridorigin, α)
    R = rotationmatrix(α)
    return R * ([i, j] * cellsize .+ cellsize / 2) .+ gridorigin
end
...
function indexes_centres(size, cellsize, celloffset, α)
    ...
    # generate array of cell centres
    centres = [
        centre(i, j, cellsize, celloffset, α) for i in UnitRange(irange...),
        j in UnitRange(jrange...)
    ]
...
function upscaleFactor(wfs::SHSensor, apertureD, λ)
    return upscaleFactor(wfs.cam.pixelsize, wfs.mla.focallength, apertureD, λ)
end
...
function make_centered_domain2D(wfs::SHSensor)
    return make_centered_domain2D(wfs.cam.imagesize..., wfs.cam.pixelsize)
end
