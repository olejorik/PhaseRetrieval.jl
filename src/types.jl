...
function PRForwardModel(name::String, ims::PhaseRetrieval.ImagingSensor; λ=633nm, diversity = [], noise = 0)
    q = PhaseRetrieval.upscaleFactor(ims, λ)
    if q > 1
    ...
    phasepixel = zero(ap)
    phaseZer= zero(ap)
    return PRForwardModel(name, ims, f, λ, d, q, roi, dualroi, ap, phasepixel, phaseZer, mask, diversity, noise)
end
